...
class UserStates(StatesGroup):
    ...
    waiting_for_scenario_check = State()  # Новое состояние для проверки сценария
...
async def check_scenario_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    scenario_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🏨 ММ гостиницы", callback_data="scenario_hotel")],
    ...
    await callback.message.answer(
        ...
        "🏨 **ММ гостиницы** - проверка соответствия сценарию гостиничного ММ\n"
...
async def handle_scenario_check_file(message: types.Message, state: FSMContext):
    ...
    scenario_type = user_data.get('scenario_type', 'scenario_hotel')
    ...
    try:
        # Импортируем функцию проверки сценария (будет реализована в другом модуле)
        from scenario_checker import check_meeting_scenario

        # Вызываем функцию проверки сценария с указанием типа
        scenario_result = await check_meeting_scenario(file_path, scenario_type)

        # Отправляем результат пользователю
        await message.answer(scenario_result)

        # Отправляем кнопку меню
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🏠 Меню", callback_data="back_to_main")]
        ])
        await message.answer("✅ Проверка завершена! Нажмите кнопку ниже для возврата в главное меню:",
                             reply_markup=keyboard)
        ...
        await message.answer("❌ Модуль проверки сценария временно недоступен. Попробуйте позже.")
        os.remove(file_path)
        await state.clear()
    except Exception as e:
        await message.answer(f"❌ Ошибка при проверке сценария: {str(e)}")
        os.remove(file_path)
        await state.clear()
